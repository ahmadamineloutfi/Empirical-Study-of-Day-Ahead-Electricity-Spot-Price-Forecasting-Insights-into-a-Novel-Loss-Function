...
def get_X_Y_B1():
    ...
    df = df.drop(df[df['Consumption Prognosis (MVH)'].isin(['NaN'])].index)
    df = df.drop(df[df['Production Prognosis (MVH)'].isin(['NaN'])].index)
    df = df.drop(df[df['Wind prognosis (MVH)'].isin(['NaN'])].index)
    df = df.drop(df[df['Prices OQF (EUR)'].isin(['NaN'])].index)
    df = df.drop(df[df['Prices OYF (EUR)'].isin(['NaN'])].index)
    df = df.drop(df[df['Brent Oil (EUR)'].isin(['NaN'])].index)
    df = df.drop(df[df['Coal Price (EUR)'].isin(['NaN'])].index)
    ...
    train_dataset = dataset.head(1400)
